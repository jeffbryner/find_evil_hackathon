...
class TriageExtractor:
    ...
    def _extract_fs_timeline(self):
        ...
        bodyfile = os.path.join(self.scratch_dir, "bodyfile.txt")
        timeline_csv = os.path.join(self.scratch_dir, "fs_timeline.csv")
        
        # Note: We need the raw image path for fls. 
        # For now, let's assume it's at /mnt/ewf/{case_name}/ewf1
        raw_image = f"/mnt/ewf/{self.case_name}/ewf1"
        
        cmd = f"bash -c 'fls -r -m / {raw_image} > /scratch/{self.case_name}/bodyfile.txt'"
        self.orchestrator.execute(cmd)
        
        cmd = f"bash -c 'mactime -b /scratch/{self.case_name}/bodyfile.txt -z UTC -d > /scratch/{self.case_name}/fs_timeline.csv'"
        self.orchestrator.execute(cmd)
    ...
    def _extract_windows_artifacts(self):
        ...
        self.orchestrator.execute(f"mkdir -p /scratch/{self.case_name}/registry")
        
        hives = ["SYSTEM", "SOFTWARE", "SAM", "SECURITY"]
        for hive in hives:
            self.orchestrator.execute(f"bash -c 'cp {self.mount_path}/Windows/System32/config/{hive} /scratch/{self.case_name}/registry/ 2>/dev/null || true'")

        # User Registry Hives
        print("[*] Extracting user registry hives...")
        self.orchestrator.execute(f"bash -c \"find {self.mount_path}/Users -name 'NTUSER.DAT' -exec cp --parents {{}} /scratch/{self.case_name}/registry/ \\;\"")
        self.orchestrator.execute(f"bash -c \"find {self.mount_path}/Users -name 'UsrClass.dat' -exec cp --parents {{}} /scratch/{self.case_name}/registry/ \\;\"")

        # Event Logs
        evtx_dir = os.path.join(self.scratch_dir, "evtx")
        os.makedirs(evtx_dir, exist_ok=True)
        self.orchestrator.execute(f"mkdir -p /scratch/{self.case_name}/evtx")
        self.orchestrator.execute(f"bash -c \"find {self.mount_path}/Windows/System32/winevt/Logs/ -name '*.evtx' -exec cp {{}} /scratch/{self.case_name}/evtx/ \\;\"")
        ...
        if offset > 0:
            self.orchestrator.execute(f"bash -c 'icat -o {offset} {raw_image} 0 > /scratch/{self.case_name}/MFT'")
        else:
            self.orchestrator.execute(f"bash -c 'icat {raw_image} 0 > /scratch/{self.case_name}/MFT'")
    ...
    def _extract_linux_artifacts(self):
        ...
        self.orchestrator.execute(f"mkdir -p /scratch/{self.case_name}/linux")
        
        files = ["/etc/passwd", "/etc/shadow", "/etc/group", "/var/log/auth.log", "/var/log/syslog"]
        for f in files:
            self.orchestrator.execute(f"cp {self.mount_path}{f} /scratch/{self.case_name}/linux/ 2>/dev/null || true")
    ...
    def _convert_to_parquet(self):
        ...
        if os.path.exists(timeline_csv):
            ...
            try:
                ...
                con.execute(f"COPY (SELECT * FROM read_csv_auto('{timeline_csv}', ignore_errors=true)) TO '{parquet_path}' (FORMAT PARQUET)")
        ...
        for file in os.listdir(self.scratch_dir):
            if file.endswith(".csv") and file != "fs_timeline.csv":
                ...
                try:
                    con.execute(f"COPY (SELECT * FROM read_csv_auto('{csv_path}', ignore_errors=true)) TO '{parquet_path}' (FORMAT PARQUET)")
                    print(f"[+] Created {parquet_path}")
                except Exception as e:
...
def main():
    ...
    # We need to find what's mounted. For now, let's assume we triage everything in /mnt/windows/
    output, _ = orchestrator.execute("ls /mnt/windows")
    mounts = output.splitlines()
    
    for mount in mounts:
        if mount.strip():
            mount_path = f"/mnt/windows/{mount.strip()}"
